...
@router.post("/", response_model=schemas.ReviewRead, summary="Add new Review record.")
def create_item(
    ...
    existing = db.query(models.Review).filter_by(ReviewId=payload.ReviewId).first()
    if existing:
        raise HTTPException(status_code=400, detail="ReviewId already exists")
    ...
    obj = models.Review(**payload.dict())
...
@router.get("/", response_model=List[schemas.ReviewRead], summary="Get list of Review records.")
def list_items(
    ...
    q = db.query(models.Review).filter(models.Review.EntityStatus != "Archived")
