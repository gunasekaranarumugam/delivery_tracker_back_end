...
@router.post("/", response_model=List[schemas.TaskTypeViewBase])
...
def list_task_type(db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"An unexpected error occurred while listing Task Type: {e}",
        )
...
def get_task_type(id: str, db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"An unexpected error occurred while fetching Task Type details: {e}",
        )
