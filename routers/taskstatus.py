...
def archive_task_status(
    ...
    # Fetch TaskStatus
    status = db.query(models.TaskStatus).filter(models.TaskStatus.status_id == id).first()
    if not status:
    ...
    return {"message": "TaskStatus archived successfully", "status_id": status.status_id}
