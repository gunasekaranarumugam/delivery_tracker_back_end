...
def update_issue_activity(id: str, payload: schemas.IssueActivityUpdate, db: Session = Depends(get_db)):
...
def archive_issue_activity(
    ...
    activity = db.query(models.IssueActivity).filter(models.IssueActivity.activity_id == id).first()
    if not activity:
    ...
    return {"message": "IssueActivity archived successfully", "activity_id": activity.activity_id}
