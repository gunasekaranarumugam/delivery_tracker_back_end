...
def create_deliverable(
    ...
    try:
        ...
        crud.audit_log(db, "Deliverable", deliverable.deliverable_id, "Create", changed_by=current_employee.employee_id)
        deliverable_view = db.query(models.DeliverableView).filter(models.DeliverableView.deliverable_id == deliverable.deliverable_id).first()
...
# ----------------------------------------------------------------------
@router.get("/", response_model=List[schemas.DeliverableRead], summary="List active Deliverables")
def list_deliverables(limit: int = 100, offset: int = 0, db: Session = Depends(get_db)):
    try:
        ...
        return db.query(models.DeliverableView).filter(
            models.DeliverableView.entity_status != "ARCHIVED"
        ).all()
        
    except (DBAPIError, OperationalError) as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Database error while fetching Deliverables list.")
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="An unexpected error occurred while listing Deliverables.")


# ----------------------------------------------------------------------
# --- GET /{id} (Get Single Deliverable) ---
# ----------------------------------------------------------------------
...
def get_deliverable(id: str, db: Session = Depends(get_db)):
    try:
        ...
        deliverable = db.query(models.DeliverableView).filter(models.DeliverableView.deliverable_id == id).first()
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Database error while fetching single Deliverable.")
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="An unexpected error occurred while fetching Deliverable.")


# ----------------------------------------------------------------------
# --- PUT /{id} (Update or Create Deliverable) ---
# ----------------------------------------------------------------------
# ----------------------------
# Update Deliverable (PUT)
# ----------------------------
"""
...
def update_deliverable(
    ...
    try:
        ...
        # Create new deliverable if it doesn't exist
        if not deliverable:
            action = "Create"
            deliverable = models.Deliverable(
                deliverable_id=id,
            ...
            # Update only fields that exist in the table (skip *_name fields)
        ...
        # Fetch the view for frontend display (with names populated)
        ...
        if not deliverable_view:
            # Fallback: return raw table data if view not found
            return deliverable
...
def update_deliverable(
    ...
    try:
        ...
        # Fetch the view for return
        deliverable_view = db.query(models.DeliverableView).filter(
            models.DeliverableView.deliverable_id == deliverable.deliverable_id
        ).first()

        print(f"Returning deliverable view: {deliverable_view.priority}")
        print(f"=== Update Deliverable Finished ===\n")
        return deliverable_view
...
def archive_deliverable(
    ...
    return {"message": "Deliverable archived successfully", "deliverable_id": deliverable.deliverable_id}
