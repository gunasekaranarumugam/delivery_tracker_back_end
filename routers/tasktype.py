...
# ----------------------------------------------------------------------
@router.post("/", response_model=schemas.TaskTypeRead, status_code=status.HTTP_201_CREATED, summary="Create a new Task Type")
def create_task_type(payload: schemas.TaskTypeCreate, db: Session = Depends(get_db),current_employee: models.Employee = Depends(get_current_employee)):
    try:
        obj = models.TaskType(
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to initialize Task Type model.")
    ...
    obj_view = db.query(models.TaskTypeView).filter(
        models.TaskTypeView.task_type_id == obj.task_type_id
    ).first()
...
def list_task_types(limit: int = Query(100, ge=1), offset: int = 0, db: Session = Depends(get_db)):
    try:
        ...
        return db.query(models.TaskTypeView).filter(
            models.TaskTypeView.entity_status != "ARCHIVED"
        ).all()
    except (DBAPIError, OperationalError) as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Database error while fetching Task Types list.")
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="An unexpected error occurred while listing Task Types.")


# ----------------------------------------------------------------------
# --- GET /{id} (Get Task Type by ID) ---
# ----------------------------------------------------------------------
...
def get_task_type(id: str, db: Session = Depends(get_db)):
    try:
        ...
        obj = db.query(models.TaskTypeView).filter(models.TaskTypeView.task_type_id == id).first()
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Database error while fetching single Task Type.")
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="An unexpected error occurred while fetching Task Type.")


# ----------------------------------------------------------------------
# --- PUT /{id} (Update Task Type record) ---
# ----------------------------------------------------------------------
@router.put("/{id}", response_model=schemas.TaskTypeRead, summary="Update Task Type record")
def update_task_type(id: str, payload: schemas.TaskTypeUpdate, db: Session = Depends(get_db),current_employee: models.Employee = Depends(get_current_employee)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Database error while retrieving Task Type for update.")
    ...
    try:
        obj_view = db.query(models.TaskTypeView).filter(models.TaskTypeView.task_type_id == obj.task_type_id).first()
...
def archive_task_type(
    ...
    return {"message": "TaskType archived successfully", "task_type_id": task_type.task_type_id}
